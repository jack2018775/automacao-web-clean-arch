...
class AtualizarParamOSSetorUseCase:
    ...
    def __call__(self, novo_setor: Setor):
        param_os_list = self.tarefa_repo.get_all_param_os()
        
        for param_os in param_os_list:
            param_os: ParamOS
            
            print(f"[INFO] Atualizando parametro os id {param_os.id}")
            
            # Filtra o parametro pelo ID
            filtrar_parametro_os_por_id = FiltrarParametroOSPorIDUseCase(self.web_automation)
            filtrar_parametro_os_por_id(param_os.id)
